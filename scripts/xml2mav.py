...
class Type:
    def __init__(self, name, size, java_repr, java_type):
        self._name = name
        self._size = size
...
class Types:
    UINT8_T = Type("uint8_t", 1, "UINT_8", "short")
    UINT16_T = Type("uint16_t", 2, "UINT_16", "int")
    UINT32_T = Type("uint32_t", 4, "UINT_32", "long")
    UINT64_T = Type("uint64_t", 8, "UINT_64", "long")
    INT8_T = Type("int8_t", 1, "INT_8", "byte")
    INT16_T = Type("int16_t", 2, "INT_16", "short")
    INT32_T = Type("int32_t", 4, "INT_32", "int")
    INT64_T = Type("int64_t", 8, "INT_64", "long")
    FLOAT = Type("float", 4, "FLOAT", "float")
    DOUBLE = Type("double", 8, "DOUBLE", "double")
    CHAR = Type("char", 1, "CHAR", "char")
...
{f'{new_line}import de.wuespace.telestion.extension.mavlink.annotation.MavArray;' if msg.contains_array() else ''}\
{f'{new_line}import de.wuespace.telestion.extension.mavlink.message.MavlinkMessage;'
    if package != 'de.wuespace.telestion.extension.mavlink.message' else ''}
...
\t}}
    ...
    for field in msg.get_fields():
        data_type = Types.get_from_string(field.get_mav_type()).get_java_type()
        array = None
...
\t * <br>
\t * <i>Autogenerated by XML2Record-Tool v{VERSION}</i>
\t */{f'{os.linesep}{tab}@MavArray(length = {array_length})' if array_length > 0 else ""}
...
{tab}@JsonProperty {data_type} {field.get_name()[:1].lower()}\
{field.get_name().lower().replace('_', ' ').title()[1:].replace(' ', '')}, /*TEMPLATE_RECORD_TYPES*/""") \
            .replace("/*TEMPLATE_CONSTRUCTOR_ARGS*/", f"\
{'null' if array_length > 0 else f'({Types.get_from_string(field.get_mav_type()).get_java_type()}) 0'}\
...
def interpret_file(file, output, package):
    ...
    print("Outputting to Java-Records (jdk-16)...")
